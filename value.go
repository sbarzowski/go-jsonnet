...
// potentialValue is something that may get evaluated to a value
...
// TODO(sbarzowski) rename to valueFunction
...
// Note that it can be used as part of composite objects
// in inheritance using operator +.
//
// Fields are late bound (to object), so they are not values or potentialValues.
// So for example we can have simple objects a = {x: 42} and b = {y: self.x}.
// Evaluating b.y is an error, but (a+b).y evaluates to 42.
...
func (f *codeUnboundField) bindToObject(sb selfBinding, origBinding bindingFrame) potentialValue {
	return makeThunk("???", makeEnvironment(origBinding, sb), f.body)
}
...
func objectFieldsVisibility(obj value) fieldHideMap {
	r := make(fieldHideMap)
	switch obj := obj.(type) {
	case *valueExtendedObject:
...
func objectFields(obj value, manifesting bool) []string {
	var r []string
...
// Misc
// -------------------------------------
// TODO(sbarzowski) if these things are "misc" then perhaps they don't belong to this file
...
type TraceContext struct {
	// Location of creation/definition.
	Loc *LocationRange
